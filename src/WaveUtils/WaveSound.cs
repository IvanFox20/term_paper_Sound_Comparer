...
namespace SoundComparer.WaveUtils
{
	...
	public class WaveSound
    {
        ...
        private WaveFormat ReadHeader(BinaryReader reader)
        {
            ...
            reader.ReadUInt32(); // File length minus first 8 bytes of RIFF description, we don't use it
            ...
            /*fmtSize -= 16; // minimum format size
            while (fmtSize > 0)
            {
                reader.ReadByte();
                fmtSize--;
            ...
            do
            {
                ...
                chunks.Add(chunk);
            } while (reader.BaseStream.Position < reader.BaseStream.Length && string.Compare(chunk, "data", true) != 0);*/
            //if (chunk != "data")
            //{
            //    throw new Exception();
            //}
        ...
        /*public float Compare(WaveSound wf)
        {
            ...
            {
                while (!endfile) // ƒобавить цикл в цикле. ¬о внешнем while пробегать по 1 файлу, во втором делать сравнение 
                {
                    try
                    {
                        a = f1.ReadInt16();
